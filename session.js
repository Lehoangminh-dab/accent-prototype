class SessionPronunciationApp {
    ...
    loadSessionData() {
        ...
        this.questionSet = urlParams.get('set');
        
        if (!this.sessionId || !this.questionSet) {
            this.feedback.textContent = 'Invalid session. Please use a valid session link.';
    ...
    async loadQuestions() {
        try {
            const response = await fetch(`questions-set/${this.questionSet}.json`);
            ...
            this.feedback.textContent = 'Error loading questions. Please check if the question set exists.';
            this.feedback.className = 'feedback incorrect';
        }
    }
